...
Elenca e descrive la sintassi e gli attributi supportati (o coppie chiave-valore) che puoi trasmettere al [!UICONTROL Data Collection Servers] ([!DNL DCS]). Queste informazioni possono essere utili per formattare [!DNL DCS] richiede e conosce i parametri restituiti da questo sistema.

## Prefissi degli attributi {#attribute-prefixes}

La [!DNL DCS] si basa su prefissi specifici aggiunti alle chiavi in coppie chiave-valore per classificare il tipo di dati che stai passando.
...
<table id="table_23B7E15EC13749E9A245DFB543822DB7"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Attributi privati definiti dal cliente. </p> <p> Il DCS accetta i propri dati privati quando la chiave ha un <code> p_</code> Prefisso. I dati privati vengono utilizzati per la valutazione delle caratteristiche, ma non verranno registrati o memorizzati nel nostro sistema. Ad esempio, supponiamo che una caratteristica sia definita come <code> customers = p_age&lt;25</code> e tu passi <code> p_age=23</code> in una chiamata evento. Considerate queste condizioni, l’utente che soddisfa i criteri di qualificazione basati sull’età si qualifica per la caratteristica, ma la coppia chiave-valore viene rilasciata dopo <span class="keyword"> Audience Manager</span> riceve la richiesta e non viene registrata. </p> </td>
  </tr> 
 </tbody> 
</table>
...
Tutte queste sono facoltative, a meno che non si desideri una risposta dal [!DNL DCS]. Se vuoi [!DNL DCS] per restituire una risposta, quindi `d_rtbd=json` è obbligatorio.
...
<table id="table_FCCE4F9D796648899772A191981EFDE6"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Specifica una funzione JavaScript da eseguire utilizzando <span class="wintitle"> DCS</span> come parametro di funzione della funzione di callback. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_cid</code> </p> </td> 
   <td colname="col2"> <p>Contiene una o più coppie di ID del provider di dati (<code> DPID</code>) e gli ID utente del provider di dati (<code> DPUUID</code>) assegnato da <span class="keyword"> Audience Manager</span>. Se utilizzi più coppie di <code> DPID</code>s e <code> DPUUID</code>s, separa ogni coppia con il carattere non stampabile <code> %01</code>. Ad esempio: <code><i>DPID</i>%01<i>DPUUUID</i></code>. </p> <p><code> d_cid</code> sostituisce <code> d_dpid</code> e <code> d_dpuuid</code>, che sono obsoleti ma sono ancora supportati. Consulta <a href="../../../reference/cid.md">CID Replaces DPID and DPUUID</a>. </p> </td>
  </tr>
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Disabilitare l'utilizzo di cookie di terze parti per rispettare le normative sulla protezione dei bambini. Questo parametro viene impostato in modo dinamico dal servizio Adobe Adobe Experience Platform Identity e dipende dal <code> idSyncDisable3rdPartySyncing</code> configurazione. Vedi <a href="https://experienceleague.adobe.com/docs/id-service/using/reference/coppa.html" format="https" scope="external"> Supporto per COPPA nel servizio Adobe Experience Platform Identity</a>. </p> </td>
  </tr>
  <tr> 
   ...
   <td colname="col2"> <p>Facoltativo. Abilitato su richiesta del cliente. Contatta il tuo consulente Adobe Audience Manager o l’Assistenza clienti. </p> <p>Indica che le caratteristiche e i segmenti devono essere restituiti all’interno della <code> JSON</code> risposta. </p> <p> 
     <ul id="ul_8B936ACB18724681B959783421ACF026"> 
      <li id="li_792A6248F49141C0B4B214C754D5F5C5"> <p><code> d_cts=1</code> return <a href="../../../reference/ids-in-aam.md"> ID di segmenti legacy</a> per i segmenti. </p> </li>
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Obsoleto. Vedi <code> d_cid</code> e <code> d_cid_ic</code>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Restituisce i dati di destinazione URL nel <code> JSON</code> risposta. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Indica la <code> JSON</code> versione da utilizzare nella risposta. Normalmente, è necessario impostare questo su <code> d_jsonv=1</code>. Impostazione <code> d_jsonv=0</code> disabilita le sincronizzazioni ID. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Obsoleto. <code> d_rs</code> è un attributo riservato, utilizzato nell’integrazione legacy tra <span class="keyword"> Adobe Analytics</span> e <span class="keyword"> Audience Manager</span>. </p> <p>Sconsigliamo di creare caratteristiche che utilizzano attributi riservati. Adobe può modificare gli attributi riservati in qualsiasi momento. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_rtbd=json</code> </p> </td> 
   <td colname="col2"> <p>Obbligatorio se si desidera un <code> JSON</code> risposta dal <span class="wintitle"> DCS</span>. </p> <p> 
     <ul id="ul_9EA00BD822504BCA8ECB59C1634DB91A"> 
      <li id="li_7CB890F92C4A4C6AA8B4EE32E1AD4564">Se ometti questo, la <span class="wintitle"> DCS</span> restituisce un pixel nell'intestazione. </li> 
      <li id="li_824C23B4C7AA4B5EBADF73D26016A18E">Se includi questo, il <span class="wintitle"> DCS</span> restituisce <code> JSON</code> nel corpo della risposta. Vedi l'esempio seguente. La tua risposta potrebbe essere più complessa. </li> 
     </ul> </p> <p> 
     <code class="syntax javascript">
      {
      &nbsp;&nbsp;&nbsp;&nbsp;"stuff":&nbsp;[],
  ...
  <tr> 
   ...
   <td colname="col2"> <p><code> SID</code> sta <span class="term"> ID punteggio</span>. Questo è un ID univoco per una caratteristica o un segmento. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Lo scopo è identico al <code> d_tdpid</code> parametro descritto sopra. Tuttavia, in questo caso, l'origine dati viene passata utilizzando il codice di integrazione. </p> <p>Tenendo le caratteristiche descritte sopra, considera la chiamata di esempio: </p> <p>Per <code>yourcompany.demdex.net/event?key1=val1&amp;key2=val2&amp;d_tdpid_ic=ic2</code>, viene restituita solo la caratteristica T2. </p> </td> 
  </tr> 
  <tr> 
...
| --- | --- | 
| `h_host` | Questo è impostato sul nome host specifico della raccolta dati del client. Viene visualizzato come `host name .demdex.net`. Consulta [Understanding Calls to the Demdex Domain](https://experienceleague.adobe.com/docs/audience-manager/user-guide/reference/demdex-calls.html?lang=en). |
| `h_user-agent` | Imposta su `User-Agent` valore di intestazione. |
| `h_accept-language` | Imposta su  `Accept-Language`  valore di intestazione. |
| `h_referer` | Imposta su `Referer` valore di intestazione. |
| `h_referrer` | Imposta su `Referrer` valore di intestazione. |
| `h_date` | Imposta su `Date` valore di intestazione. |