...
# Fatturazione per gli acquirenti di feed di dati {#billing-for-data-feed-buyers}
 ...
 acquirenti di dati di Audience Marketplace accettano di segnalare tutte le impression pubblicitarie servite utilizzando le caratteristiche contenute nel feed di dati a un prezzo per migliaia di annunci ([!DNL CPM]). [!DNL CPM] l’utilizzo è previsto il 5° giorno di ogni mese di calendario e include i dati per il mese precedente. Gli abbonati a tariffa fissa non devono segnalare l&#39;utilizzo.
...
## Rapporto Utilizzo CPM a livello di segmento {#segment-level-report}

La scheda [!UICONTROL Segment Usage] consente di creare rapporti sull&#39;utilizzo a livello di segmento, mentre i segmenti vengono visualizzati raggruppati in base alle destinazioni a cui sono mappati.

Dopo aver segnalato l&#39;utilizzo di [!DNL CPM] a livello di segmento, [!UICONTROL Audience Marketplace] assegna automaticamente i feed di dati corrispondenti al corretto utilizzo, in base all&#39; [Attribuzione costi per feed di dati CPM](#cost-attribution).
...
1. Compila l’utilizzo per i tuoi segmenti. È possibile utilizzare la casella [!UICONTROL Search] per filtrare i segmenti se è necessario solo segnalare l&#39;utilizzo di alcuni di essi.
1. Clic **[!UICONTROL Edit Segments Usage]**.
1. Immettere l&#39;importo dell&#39;utilizzo [!DNL CPM] nella colonna [!UICONTROL Usage].
1. Fare clic su **[!UICONTROL Save]** al termine dell&#39;operazione e rivedere la finestra di dialogo di conferma.
...
## Rapporto Utilizzo CPM a livello di feed di dati {#feed-level-report}

La generazione di rapporti a livello di feed di dati è un processo più tedioso e suscettibile di errori, in quanto è necessario calcolare singolarmente l&#39;utilizzo di [!DNL CPM] per ogni feed di dati. È consigliabile [Segnalare l&#39;utilizzo di CPM a livello di segmento](#segment-level-report).

Per segnalare l&#39;utilizzo di [!DNL CPM] a livello di segmento:

1. Vai a **[!UICONTROL Audience Marketplace > Payables]**.
2. Selezionare la scheda **[!UICONTROL Feed Usage]**.
3. Utilizzate la casella [!UICONTROL Search] per filtrare i feed di dati e identificare quelli per i quali è necessario segnalare l&#39;utilizzo.
4. Clic **[!UICONTROL Edit Feeds Usage]**.
5. Calcolare l&#39;utilizzo di [!DNL CPM] per ciascun feed di dati in base all&#39; [Attribuzione costi per feed di dati CPM](#cost-attribution), quindi immetterlo nella colonna [!UICONTROL Usage].
6. Fare clic su **[!UICONTROL Save]** al termine dell&#39;operazione e rivedere la finestra di dialogo di conferma.
...
Per ridurre gli errori e il sovraccarico durante la segnalazione dell&#39;utilizzo [!DNL CPM], potete utilizzare l&#39;opzione di reporting in blocco per scaricare un file [!DNL CSV] contenente i feed di dati e i segmenti, riempire l&#39;utilizzo e caricarlo nuovamente in [!DNL Audience Manager]. Potete utilizzare la generazione di rapporti in massa per segnalare sia l&#39;utilizzo dei feed che quello dei segmenti.

Per aggiornare l&#39;utilizzo di [!DNL CPM] in blocco:

1. Vai a **[!UICONTROL Audience Marketplace > Payables]**.
1. Selezionare la scheda **[!UICONTROL Feed Usage]** o **[!UICONTROL Segment Usage]**, a seconda del tipo di rapporto che si desidera aggiornare.
1. Fare clic su **[!UICONTROL Edit Feeds Usage]** o **[!UICONTROL Edit Segments Usage]**.
1. Fate clic su **[!UICONTROL download the current usage]** per essere certi di usare un file CSV valido.
1. Aprite il file sul computer e compilate il rapporto di utilizzo.
1. Fate clic su **[!UICONTROL Choose a CSV file]** per caricare il rapporto di utilizzo aggiornato.
...
| Valori non supportati | [!DNL Audience Manager] rilevati valori non numerici nella  [!DNL Audience Manager] colonna. | Rivedete il rapporto e accertatevi di immettere solo valori numerici nella colonna [!DNL Audience Manager]. |
| Intestazioni per campi obbligatori mancanti | [!DNL Audience Manager] rilevate intestazioni di tabella mancanti per i campi obbligatori. Per [!UICONTROL Segment Level Reporting], i campi obbligatori sono: [!UICONTROL Segment ID], [!UICONTROL Destination ID]. Per [!UICONTROL Feed Level Reporting], i campi obbligatori sono: [!UICONTROL Data Provider Name], [!UICONTROL Data Feed Name], [!UICONTROL Use Case] | Rivedete il rapporto e accertatevi che le intestazioni della tabella non siano state alterate. |

>[!NOTE]
>La rimozione delle righe dal report sull&#39;uso [!DNL CSV] non ha alcun effetto sul report sull&#39;utilizzo esistente. [!DNL Audience Manager] elabora solo i campi inclusi nel rapporto.
...
In [!UICONTROL Audience Marketplace] è necessario generare automaticamente rapporti sugli importi mensili, per ogni segmento. È consigliabile riportare l&#39;utilizzo [!DNL CPM] a livello di segmento, in modo che l&#39;attribuzione dei costi venga eseguita automaticamente.

<!-- marketplace_cpm_billing.xml -->

### Riepilogo fatturazione {#billing-summary}

È necessario inviare [!DNL CPM] gli importi delle impressioni dei feed di dati tra il primo e il quinto giorno di ogni mese di calendario. A questo scopo, si consiglia di [Segnalare l&#39;utilizzo di CPM a livello di segmento](#segment-level-report).
...
Se è necessario [!UICONTROL Report CPM Usage at Data Feed Level], è necessario compilare singolarmente tutte le impression consegnate per ciascun feed nel mese di calendario precedente e segnalarle in base all&#39;allocazione di fatturazione descritta in questo articolo.

Dopo aver riportato il numero [!DNL CPM] per il mese di calendario precedente, [!DNL Adobe] esegue le seguenti operazioni:

* Crea una fattura e una fattura in base al tasso [!DNL CPM] per ciascun feed di dati sottoscritto.
* Paga le tariffe (venditori) dovute in base all&#39;utilizzo segnalato [!DNL CPM].
...
Il caso d&#39;uso [!UICONTROL Activation] consente di utilizzare caratteristiche nel feed di dati corrispondente per creare segmenti in [Generatore di segmenti](../../../features/segments/segment-builder.md) e mappare tali segmenti a una destinazione. Gli operatori booleani [!UICONTROL AND], [!UICONTROL OR] e [!UICONTROL NOT] consentono di impostare le condizioni per la qualificazione di caratteristiche e segmenti.

Quando si [Report CPM Usage at Data Feed Level](#feed-level-report) (Segnala utilizzo CPM a livello di feed di dati), è necessario allocare le impression in modo proporzionale per ciascun feed di dati, in base agli operatori [!DNL Boolean] utilizzati nelle regole di qualifica delle caratteristiche. Nella tabella seguente è illustrato come allocare correttamente le impression per regola booleana o tipo di caratteristica.

>[!TIP]
>[Segnala l’utilizzo di CPM a livello di segmento](#segment-level-report) affinché il rapporto sul livello di feed di dati venga eseguito automaticamente da  Audience Manager.
...
<table id="table_BF00FE6740D2459DAFA62F2478492586"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Applica il 100% dei totali impression consegnati a tutte le caratteristiche del fornitore in un segmento basato su regole che utilizza una condizione booleana <span class="wintitle"> E</span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Applicare il 100% dei totali impression consegnati a tutte le caratteristiche del fornitore in un segmento basato su regole che utilizza una condizione booleana <span class="wintitle"> NOT</span>. </p> </td> 
  </tr> 
  <tr> 
...
Gli esempi riportati di seguito illustrano come [!DNL CPM] l&#39;allocazione dell&#39;utilizzo viene eseguita a livello di feed di dati.

>[!IMPORTANT]
>È invece consigliabile [Segnalare l&#39;utilizzo di CPM a livello di segmento](#segment-level-report), affinché questo processo venga eseguito automaticamente.
...
Questo segmento contiene 3 caratteristiche di provider di dati separati. Poiché la qualifica del segmento è basata su una condizione [!UICONTROL AND], i visitatori devono realizzare le caratteristiche di tutti e tre i feed per qualificarsi per il segmento.

![](assets/billing-segment-and.png)

Con una condizione [!UICONTROL AND], devi assegnare il 100% delle impression ricevute durante il mese a tutti e tre i provider di dati. Nella sezione [!UICONTROL Audience Marketplace > Payables], accreditate ogni provider con 1.000.000 impression.
...
Questo segmento contiene 3 caratteristiche di provider di dati separati. Poiché la qualifica del segmento è basata su una condizione [!UICONTROL OR], i visitatori devono realizzare almeno una delle tre caratteristiche per qualificarsi per il segmento.

Non possiamo dire quale caratteristica è responsabile di un&#39;impressione perché la qualifica si basa su una condizione [!UICONTROL OR]. Di conseguenza, nella sezione [!UICONTROL Audience Marketplace > Payables] viene accreditato ogni fornitore con una ripartizione ponderata delle impression totali, in base alla popolazione di caratteristiche.
...
Il segmento viene mappato su una destinazione e per questo segmento vengono immesse 1.000.000 impression in un mese, utilizzando [Segment-Level Reporting](#segment-level-report).
