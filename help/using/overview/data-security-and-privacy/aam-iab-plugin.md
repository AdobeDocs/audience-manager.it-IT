...
>[!IMPORTANT]
>
>L&#39;Audience Manager è registrato nel [IAB TCF](https://iabeurope.eu/tcf-for-vendors/) con l&#39;ID fornitore 565.
...
Clienti che eseguono l’aggiornamento dei [!DNL Audience Manager Plug-in for IAB TCF] implementazione da [!DNL IAB TCF] da v1.1 a [!DNL IAB TCF] v2.0 o abilitazione [!DNL IAB TCF] v2.0 per la prima volta, segui tutte le stesse linee guida sui prerequisiti e l’implementazione come descritto di seguito.
...
>
> I clienti che desiderano continuare a utilizzare il plug-in di Audience Manager per IAB TCF per la gestione del consenso devono effettuare l’aggiornamento alla versione più recente di [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) per un supporto continuo.
>
> Dopo l’aggiornamento alla versione più recente [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) le stringhe di consenso IAB TCF v1.1 non saranno più supportate, quindi assicurati di aggiornare la CMP prima di eseguire l’aggiornamento alla versione ECID più recente.
...
2. Devi utilizzare Audience Manager [!DNL Data Integration Library] (DIL) versione 9.0 o successiva, scaricabile da [qui](https://github.com/Adobe-Marketing-Cloud/dil/releases). Leggi informazioni sulla [DIL nella documentazione di Audience Manager](../../dil/dil-overview.md). Si consiglia di utilizzare [Estensione tag Adobe Audience Manager](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/audience-manager/overview.html) per l’implementazione DIL più semplice di Audience Manager.
...
Il modo più semplice per farlo è utilizzare [Tag Adobe Experience Platform](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=en) per aggiungere [!DNL ECID Opt-in] sulle proprietà. Leggi la documentazione per [Estensione Opt-in ECID](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html) per scoprire come impostare l’estensione Tag.
...
Gli utenti forniscono le proprie scelte sotto forma di *consenso* e *interesse legittimo* ai fini dell’IAB *fornitori di terze parti* registrati nell&#39;elenco dei fornitori globali.
...
I dettagli sulle varie finalità e autorizzazioni incluse in IAB TCF v2.0 sono trattati nel [Politiche del quadro di riferimento per la trasparenza e il consenso in IAB Europe](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes).
...
L&#39;Audience Manager valuta le scelte degli utenti memorizzate nella stringa IAB TC per i seguenti scopi, definiti nella [Politiche del quadro di riferimento per la trasparenza e il consenso in IAB Europe](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Appendix_A_Purposes_and_Features_Definitions).
...
4. Se il RGPD è applicabile, l’Audience Manager controlla la stringa IAB TC, trasmessa nel `gdpr_consent` , per il consenso richiesto. Audience Manager richiede il consenso per la memorizzazione e/o l&#39;accesso alle informazioni su un dispositivo ([Scopo 1 IAB TCF](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)), sviluppo e miglioramento dei prodotti ([Scopo IAB TCF 10](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)), oltre al consenso Audience Manager del fornitore per archiviare, elaborare o attivare i dati.
5. Se la stringa IAB TC è presente e contiene il consenso richiesto, l&#39;Audience Manager trasmette la stringa IAB TC al nostro [server di raccolta dati](../../reference/system-components/components-data-collection.md) (DCS).
6. L&#39;Audience Manager risponde impostando un [cookie demdex](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-am.html) nel browser e avvia e rispetta le sincronizzazioni ID di terze parti.
...
3. Se la stringa IAB TC è presente e contiene le autorizzazioni richieste, Audience Manager memorizza i dati passati nel `gdpr` e `gdpr_consent` variabili nelle chiamate pixel.
4. Se la stringa IAB TC è mancante o mancano le autorizzazioni richieste, l&#39;Audience Manager rilascia i dati passati nel `gdpr` e `gdpr_consent` variabili nelle chiamate pixel.
...
L’integrazione di Audience Manager con IAB TCF v2.0 supporta l’aggiunta del consenso alle informazioni inviate a [Destinazioni URL](../../features/destinations/create-url-destination.md) che sono integrati con IAB TCF v2.0. Tuttavia, questo processo non viene eseguito automaticamente per Audience Manager, per evitare di interrompere formati URL specifici.

Clienti che desiderano aggiungere il consenso ai dati inviati a [!DNL URL destinations] deve aggiungere manualmente `${GDPR}` e `${GDPR_CONSENT_XXXX}` macro nel formato URL, sostituendo `XXXX` con l&#39;ID partner di destinazione.

Esempio: `https://yourdomain.com?gdpr=${GDPR}&gdpr_consent=${GDPR_CONSENT_1234}`.

Vedi [Definizione delle macro delle destinazioni](../../features/destinations/destination-macros.md) per ulteriori informazioni sulle macro di destinazione supportate.

## Gestione del consenso tra dispositivi

Il plug-in di Audience Manager per IAB TCF rifiuta automaticamente gli ID presenti su una richiesta, quando i visitatori del sito non forniscono le autorizzazioni appropriate. Se la richiesta contiene un [ID multi-dispositivo (CRM ID)](../../reference/ids-in-aam.md), Audience Manager rinuncia all’ID, insieme all’ultimo dispositivo collegato a quello [ID multi-dispositivo (CRM ID)](../../reference/ids-in-aam.md).

## Testare l’implementazione IAB {#test-iab-implementation}

Per verificare di aver implementato correttamente il plug-in di Audience Manager per IAB TCF, leggi [Caso d&#39;uso 4 nella convalida del servizio Opt-in](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/testing-optin-and-iab-plugin.html#section-64331998954d4892960dcecd744a6d88).
