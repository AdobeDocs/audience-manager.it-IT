   ...
   <td colname="col2"> <p>L’ID <span class="term"> del provider di</span> dati (DPID, Data Provider ID <span class="keyword"> ) è un identificatore che informa</span> Audience Managerse un file di dati contiene ID utente o ID Android o iOS personali. Accetta le seguenti opzioni: </p> <p> <b>ID partner dati</b> </p> <p> Si tratta di un ID univoco assegnato da Audience Manager alla tua società o organizzazione. Utilizzate questo ID assegnato in un nome file quando inviate dati che contengono i vostri ID utente. Ad esempio, <code>...ftp_dpm_21_123456789.sync</code> indica <span class="keyword"> ad Audience Manager</span> che un partner con ID 21 ha inviato il file e contiene gli ID utente assegnati da tale partner. </p> <p> <b>ID Android (GAID)</b> </p> <p> Utilizzate ID 20914 come DPID in un nome file di dati se il file contiene ID Android. Se utilizzi ID 20914 come DPID, devi comunque identificare la tua società in <span class="keyword"> Audience Manager</span>. Questo significa che il nome file deve usare il <code><i>_DPID_TARGET_DATA_OWNER</i></code> parametro per contenere l'ID società. Ad esempio, supponiamo che vengano inviati file con ID Android e che il tuo ID provider dati sia 21. In questo caso, il nome del file sarà simile <code>...ftp_dpm_20914_21_123456789.sync</code>. Questo indica ad <span class="keyword"> Audience Manager</span> che il file contiene ID Android ed è di un partner identificato dall'ID 21. </p> <p> <b>ID iOS (IDFA)</b> </p> <p> Utilizzate ID 20915 come DPID in un nome file di dati se il file contiene ID iOS. Se utilizzi ID 20915 come DPID, devi comunque identificare la tua società in <span class="keyword"> Audience Manager</span>. Questo significa che il nome file deve usare il <code><i>_DPID_TARGET_DATA_OWNER</i></code> parametro per contenere l'ID società. Ad esempio, supponiamo che vengano inviati file con ID Android e che il tuo ID provider dati sia 21. In questo caso, il nome del file sarà simile <code>...ftp_dpm_20915_21_123456789.sync</code>. Questo indica ad <span class="keyword"> Audience Manager</span> che il file contiene ID iOS ed è di un partner identificato dall'ID 21. </p> 
    <draft-comment> 
     <ul id="ul_818EB3EB2E5543F0B048BCEBB6699562"> 
      ...
      <li id="li_1955911BA11F4F458227B77F383F25A3"> <b></b> ID Android (GAID): Usa l'ID 20914 in un nome file di dati se contiene un ID Android. Ad esempio, <code>...ftp_dpm_20914_21_123456789.sync</code> indica <span class="keyword"> ad Audience Manager</span> che il file di dati contiene solo ID Android. Nota: ID 21 </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Opzioni di sincronizzazione che includono: </p> <p> 
     <ul id="ul_DAAF61EC636C4456BECDDC34C3F86E83"> 
      ...
      <li id="li_8FE8430C2C004F87835D55231A0D99C9"> <code> overwrite</code>: Consente ai provider di dati di inviare un elenco di caratteristiche in base agli utenti, che dovrebbero sovrascrivere tutte le caratteristiche di terze parti esistenti per questo provider di dati in Audience Manager. Non è necessario includere tutti gli utenti in un file di sovrascrittura. Includete solo gli utenti che desiderate modificare. </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
