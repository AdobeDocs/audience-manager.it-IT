...
Quando crei i file in entrata, assicurati che la prima colonna sia compilata solo con gli ID dispositivo, ad esempio [!DNL AAM UUID], [!DNL GAID], [!DNL IDFA]e così via. Consulta [Indice degli ID in Audience Manager](../../../reference/ids-in-aam.md) per una spiegazione dettagliata degli ID supportati da Audience Manager.

>[!IMPORTANT]
>
>Non usi [DPUUID](../../../reference/ids-in-aam.md) nella prima colonna. Questa operazione può causare una sincronizzazione errata.

## La sincronizzazione corrisponde ai DPUUID agli UUID {#sync-matches-dpuuids-uuids}

Un file di sincronizzazione ID ha lo scopo di sincronizzare [DPUUID](../../../reference/ids-in-aam.md) dalle tue origini dati con [!DNL Audience Manager] UUID. La sincronizzazione mappa [!DNL DPUUID]s dal master [!DNL DPID] e relative [!DNL DPID]s al [!DNL Audience Manager] [!DNL UUID]s. Il punto in cui inserisci gli ID nel nome e nel corpo del file determina il modo in cui questi identificatori vengono mappati l’uno sull’altro. Ad esempio, prendi i due file di esempio mostrati qui:
...
**File 1** ( [Scarica file di esempio](assets/adobe_id_0_12345_1476312152.sync))
...
Passaggio 1: il processo di sincronizzazione ID sincronizzerà [!DNL DPUUID]s da [!DNL DPID] 12345 con [!DNL Audience Manager] [!DNL UUID]s nella colonna sinistra. Tieni presente che [!DNL DPID] &quot;0&quot; nel nome del file rappresenta [!DNL Audience Manager] [!DNL UUID]s.
<br/>

**File 2** ( [Scarica file di esempio](assets/adobe_id_12345_67890_1477846458.sync))
...
Passaggio 2: [!DNL DPUUID]s da [!DNL DPID] 12345 stati sincronizzati nel passaggio 1 con l’Audience Manager [!DNL UUID]s. Questa sincronizzazione ID eseguirà la sincronizzazione di [!DNL DPUUID]s da [!DNL DPID] 67890 con l’Audience Manager [!DNL UUID]s dal passaggio 1.
...
* Utilizzare [!DNL URL] codifica. Passa solo ID non codificati.
