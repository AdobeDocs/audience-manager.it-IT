...
title: 'Contenuto del file di dati in entrata: sintassi, caratteri non validi, variabili ed esempi'
...
# Contenuto del file di dati in entrata: Sintassi, caratteri, variabili ed esempi non validi {#inbound-data-file-contents-syntax-invalid-characters-variables-and-examples}

Campi, sintassi e regole obbligatori da seguire per la formattazione di un file di dati con caratteristiche in entrata.

## Sintassi dei contenuti dei file {#file-content-syntax}
...
Per altri formati di file accettati, consulta [Integrazioni di partner personalizzate](/help/using/integration/sending-audience-data/custom-partner-integrations.md).
...
## Definizione delle variabili di file {#file-variables-defined}
...
Nella tabella seguente sono descritti i prefissi che identificano [!UICONTROL trait] nomi o ID in un file di dati in entrata. Consulta la sezione [file di esempio](../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md#data-file-examples) per esempi.
...
<table id="table_AD54B3E5487E47C481A4E5FD3A93FDA5"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>La <code> d_sid </code> Il prefisso indica al nostro sistema che l’ID è un <span class="keyword"> Audience Manager </span> ID caratteristica. Questo è lo stesso ID visualizzato nell'interfaccia utente. Puoi anche restituire gli ID delle caratteristiche con l’API <code> GET </code> metodo . Vedi <a href="../../../api/rest-api-main/api-traits.md"> Metodi API per le caratteristiche </a>. </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p> <code> d_unsid= </code> </p> </td> 
   <td colname="col2"> <p>Dati con prefisso <code> d_unsid </code> rimuove gli utenti da quella caratteristica. La <code> d_unsid </code> Il prefisso viene ignorato in un <code> overwrite </code> file. </p> <p>La <code> d_unsid= </code> Il prefisso indica al nostro sistema che l’ID è un <span class="keyword"> Audience Manager </span> ID caratteristica. Questo è lo stesso ID visualizzato nell'interfaccia utente. Puoi anche restituire gli ID delle caratteristiche con l’API <code> GET </code> metodo . Vedi <a href="../../../api/rest-api-main/api-traits.md"> Metodi API per le caratteristiche </a>. </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> ic= </code> </p> </td> 
   <td colname="col2"> <p> <a href="../../../features/traits/manage-trait-rules.md#managing-trait-rules"> Regole sulle caratteristiche </a> consente di impostare criteri per la qualifica delle caratteristiche. Se formatti una regola delle caratteristiche come <code> ic == trait ID </code>, puoi inviare le caratteristiche in un semplice elenco in formato virgola. </p> <p>Ad esempio, supponiamo che tu crei queste 3 regole di caratteristiche: </p> <p> 
     <ul class="simplelist"> 
      <li> <code> ic == "123" </code> </li>
      <li> <code> ic == "456" </code> </li>
      <li> <code> ic == "789" </code> </li>
     </ul> </p> <p>Queste caratteristiche sono associate al <code> ic </code> chiave. Questo ti consente di creare un elenco di caratteristiche più semplice nel file di dati. Inoltre, non è necessario includere il <code> ic </code> Prefisso. Come risultato, il contenuto del file di dati potrebbe essere simile al seguente: </p> <p>
     <code> 
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
...
[!UICONTROL Trait IDs] sono costituiti solo da caratteri numerici. Ti chiediamo di includere *only[!UICONTROL onboarded traits]* in file di dati in entrata. Non elaboriamo nessun altro [!UICONTROL trait] tipi nel trasferimento di dati in entrata.
...
<table id="table_8C5C7271B813441EA2D45CA2FE2A6C59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><i>Non</i> usa due punti codificati ( <code> %3A </code>) o due punti non codificati ( : ) in DPUUID. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_41A4991090A64DEFA9AF704164B26DBB"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <code> TAB </code> </p> </td> 
   <td colname="col2"> <p><i>Non</i> use <code> TAB </code> invece di valori vuoti nelle coppie chiave-valore. Solo utilizzo <code> TAB </code> per separare le variabili nel file di dati in entrata. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_8017E070F7A54143A82CA153CBAEB5DA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Queste caratteristiche sono state aggiunte a una regola delle caratteristiche con <code> ic </code> Prefisso. Di conseguenza, puoi aggiungerli al file di dati separati da virgole come mostrato. Una scheda separa l’UUID e gli ID delle caratteristiche. La <code> ic </code> Prefisso non obbligatorio nel file. </p> <p><b>ID numerici</b> </p> <p> 
     <code>
...
[Scarica](assets/ftp_dpm_1234_1445374061.overwrite) il file di dati di esempio, se hai bisogno di ulteriori esempi. Il file di download ha un `.overwrite` estensione file. Puoi aprirlo con un semplice editor di testo.

## Matrice di esempi {#examples-matrix}

Il grafico seguente mostra alcuni esempi del modo corretto di formattare i file in entrata, a seconda del [tipo di ID](../../../reference/ids-in-aam.md) e il metodo con cui si desidera aggiungere [!UICONTROL traits] ai profili.
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL Audience Manager] [!DNL UUIDs].
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sull&#39;interdizione [!DNL Audience Manager] [!DNL UUIDs].
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica [!DNL Audience Manager] [!DNL UUIDs].
...
Utilizza la `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL Audience Manager] [!DNL UUIDs].
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL Android] dispositivi.
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sull&#39;interdizione [!DNL Android] dispositivi.
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica [!DNL Android] dispositivi.
...
Utilizza la `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL Android] dispositivi.
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL iOS] dispositivi.
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sull&#39;interdizione [!DNL iOS] dispositivi.
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica [!DNL iOS] dispositivi.
...
Utilizza la `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL iOS] dispositivi.
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL DPUUIDs].
...
Utilizzo [!UICONTROL trait IDs] per inviare [!UICONTROL trait] informazioni sull&#39;interdizione [!DNL DPUUIDs].
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica [!DNL DPUUIDs].
...
Utilizza la `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica [!DNL DPUUIDs].
