...
I campi nel file di dati in entrata devono essere visualizzati nell’ordine indicato di seguito. In questo esempio, la proprietà `<` `>` sono stati aggiunti dei simboli per separare visivamente ogni elemento. Non è necessario includerli nel file di dati.

```
<user ID><TAB><trait ID>,<trait ID>,<trait ID>,...
```

Per altri formati di contenuto di file accettati, vedi [Integrazioni di partner personalizzate](/help/using/integration/sending-audience-data/custom-partner-integrations.md).
...
<table id="table_FE043CE392B34D5194111188E5C39671"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Un ID utente può essere: </p> <p> 
     <ul id="ul_25168355353545A9A049D0083403025E"> 
      ...
      <li id="li_52ABF6CCBCD147E2BD84D056F7461BA0">Un ID dispositivo mobile Android o iOS nella sua forma originale, non modificata, come esposto dal sistema operativo mobile. </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Il <span class="keyword"> Audience Manager </span> ID caratteristica. Ti chiediamo di includere <i>Solo caratteristiche onboarded</i> nei file di dati in entrata. Non elaboriamo altri tipi di caratteristiche nel trasferimento di dati in entrata. </p> <p> <p>Nota: l’ID caratteristica può essere trovato utilizzando il metodo GET che restituisce i dettagli di tutte le caratteristiche. Per ulteriori informazioni, consulta <a href="../../../api/rest-api-main/api-traits.md"> Metodi API per le caratteristiche </a>. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
Nella tabella seguente vengono descritti i prefissi che identificano [!UICONTROL trait] nomi o ID in un file di dati in entrata. Consulta la [file di esempio](../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md#data-file-examples) ad esempio.
...
<table id="table_AD54B3E5487E47C481A4E5FD3A93FDA5"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Il <code> d_sid </code> il prefisso comunica al nostro sistema che l’ID è un <span class="keyword"> Audience Manager </span> ID caratteristica. Questo è lo stesso ID visualizzato nell’interfaccia utente di. Puoi anche restituire gli ID delle caratteristiche con l’API <code> GET </code> metodo. Consulta <a href="../../../api/rest-api-main/api-traits.md"> Metodi API per le caratteristiche </a>. </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p> <code> d_unsid= </code> </p> </td> 
   <td colname="col2"> <p>Dati con prefisso <code> d_unsid </code> rimuove gli utenti da quella caratteristica. Il <code> d_unsid </code> il prefisso viene ignorato in un <code> overwrite </code> file. </p> <p>Il <code> d_unsid= </code> il prefisso comunica al nostro sistema che l’ID è un <span class="keyword"> Audience Manager </span> ID caratteristica. Questo è lo stesso ID visualizzato nell’interfaccia utente di. Puoi anche restituire gli ID delle caratteristiche con l’API <code> GET </code> metodo. Consulta <a href="../../../api/rest-api-main/api-traits.md"> Metodi API per le caratteristiche </a>. </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> ic= </code> </p> </td> 
   <td colname="col2"> <p> <a href="../../../features/traits/manage-trait-rules.md#managing-trait-rules"> Regole delle caratteristiche </a> consente di impostare i criteri per la qualifica delle caratteristiche. Se si formatta una regola di caratteristiche come <code> ic == trait ID </code>, puoi inviare le caratteristiche in un semplice elenco formattato con virgole. </p> <p>Ad esempio, supponiamo che tu crei queste 3 regole per le caratteristiche: </p> <p> 
     <ul class="simplelist"> 
      <li> <code> ic == "123" </code> </li>
      <li> <code> ic == "456" </code> </li>
      <li> <code> ic == "789" </code> </li>
     </ul> </p> <p>Queste caratteristiche sono associate al <code> ic </code> chiave. Questo consente di creare un elenco di caratteristiche più semplice nel file di dati. Inoltre, non è necessario includere <code> ic </code> prefisso. Di conseguenza, il contenuto del file di dati potrebbe essere simile al seguente: </p> <p>
     <code> 
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
...
[!UICONTROL Trait IDs] sono costituite solo da caratteri numerici. Ti chiediamo di includere *solo[!UICONTROL onboarded traits]* nei file di dati in entrata. Non elaboriamo nessun altro [!UICONTROL trait] tipi nel trasferimento di dati in entrata.
...
<table id="table_8C5C7271B813441EA2D45CA2FE2A6C59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><i>Do not</i> usa due punti codificati ( <code> %3A </code>) o due punti non codificati ( : ) nei DPUUID. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_41A4991090A64DEFA9AF704164B26DBB"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <code> TAB </code> </p> </td> 
   <td colname="col2"> <p><i>Do not</i> utilizzare <code> TAB </code> invece di valori vuoti nelle coppie chiave-valore. Usa solo <code> TAB </code> per separare le variabili nel file di dati in entrata. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_8017E070F7A54143A82CA153CBAEB5DA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Queste caratteristiche sono state aggiunte a una regola di caratteristiche con <code> ic </code> prefisso. Di conseguenza, puoi aggiungerli al file di dati separati da virgole, come illustrato nella figura. Una scheda separa l’UUID e gli ID delle caratteristiche. Il <code> ic </code> prefisso non richiesto nel file. </p> <p><b>ID numerici</b> </p> <p> 
     <code>
  ...
  <tr> 
   ...
   <td colname="col2"> Questi dati del file utilizzano coppie chiave-valore per trasmettere i dati a <span class="keyword"> Audience Manager </span>. <p> 
     <code>
...
[Scarica](assets/ftp_dpm_1234_1445374061.overwrite) il file di dati di esempio, se sono necessari ulteriori esempi. Il file di download ha un `.overwrite` estensione file. Puoi aprirlo con un semplice editor di testo.

## Matrice di esempi {#examples-matrix}

Il grafico seguente mostra alcuni esempi del modo corretto di formattare i file in entrata, a seconda [tipo di ID](../../../reference/ids-in-aam.md) e il metodo da aggiungere [!UICONTROL traits] ai profili.
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL Audience Manager] [!DNL UUIDs].
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sull’interdizione per [!DNL Audience Manager] [!DNL UUIDs].
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica per [!DNL Audience Manager] [!DNL UUIDs].
...
Utilizza il `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL Audience Manager] [!DNL UUIDs].
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL Android] dispositivi.
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sull’interdizione per [!DNL Android] dispositivi.
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica per [!DNL Android] dispositivi.
...
Utilizza il `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL Android] dispositivi.
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL iOS] dispositivi.
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sull’interdizione per [!DNL iOS] dispositivi.
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica per [!DNL iOS] dispositivi.
...
Utilizza il `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL iOS] dispositivi.
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL DPUUIDs].
...
Utilizzare [!UICONTROL trait IDs] da inviare [!UICONTROL trait] informazioni sull’interdizione per [!DNL DPUUIDs].
...
Inviare coppie chiave-valore da aggiungere [!UICONTROL trait] informazioni sulla qualifica per [!DNL DPUUIDs].
...
Utilizza il `ic` prefisso da inviare [!UICONTROL trait] informazioni sulla qualifica per [!DNL DPUUIDs].
