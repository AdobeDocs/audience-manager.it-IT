---
description: Panoramica di alto livello sulle modalità  Audience Manager di scambio di informazioni con altri fornitori e sistemi di dati.
seo-description: Panoramica di alto livello sulle modalità  Audience Manager di scambio di informazioni con altri fornitori e sistemi di dati.
...
Panoramica di alto livello sulle modalità  Audience Manager di scambio di informazioni con altri fornitori e sistemi di dati.
...
* **Real-Time:** trasferisce immediatamente i dati quando un utente visita il sito. Questo metodo è noto anche come integrazione *`synchronous`*.
* **Batch ([!DNL Server-to-Server]):** Trasferisce i dati tra i server in una pianificazione prestabilita dopo che un visitatore ha lasciato la pagina. Questo metodo è noto anche come integrazione *`out-of-band`* o *`asynchronous`*.

## Prerequisiti: Creare una tassonomia delle caratteristiche {#prereqs}

Prima dell&#39;inizio del processo di integrazione, ricordare di [creare caratteristiche](../features/traits/create-onboarded-rule-based-traits.md) e una [struttura di cartelle](../features/traits/trait-storage.md#create-trait-storage-folder) nell&#39;interfaccia [!DNL Audience Manager]. La tassonomia conterrà tutte le [!UICONTROL traits] organizzate in una gerarchia logica.

## Casi di utilizzo dell&#39;integrazione {#integration-use-cases}

Un riepilogo caso d&#39;uso dei metodi di integrazione dei dati  Audience Manager con i vantaggi e gli svantaggi di ciascuno.

### Integrazioni in tempo reale [!DNL Server-to-Server]

<!-- c_int_types_use_cases.xml -->

Un&#39;integrazione di dati in tempo reale [!DNL server-to-server] sincronizza rapidamente i dati utente tra  server di Audience Manager e un altro sistema di targeting. Nella maggior parte dei casi, lo scambio di dati avviene entro pochi secondi o minuti, a seconda della frequenza di aggiornamento del sistema di targeting. Tuttavia, il sistema di destinazione determina questo intervallo di aggiornamento, non  Audience Manager. Inoltre, la frequenza di aggiornamento può variare da un sistema all&#39;altro. L&#39;integrazione [!UICONTROL server-to-server] in tempo reale è il tipo di integrazione preferito per lo scambio di dati.  Audience Manager utilizza questo metodo ogni volta che i partner di targeting possono supportarlo.
 ...
 <tr class="strow">
  ...
  <td class="stentry"> 
   <ul id="ul_F251AFF8A2FA49D0849E36D7FAE87DE7"> 
    ...
    <li id="li_046BF4568B104F53A0E5372568C957CD">Consente di eseguire rapidamente il targeting in base al tempo per intervenire su un utente qualificato. </li>
...
Un&#39;integrazione batch [!DNL server-to-server] raccoglie i dati e li invia ad altri sistemi a intervalli prestabiliti anziché in tempo quasi reale. Gli intervalli di trasferimento dei dati iniziano da 24 ore. Alcuni provider di dati supportano solo questo tipo di integrazione. Tuttavia, abbiamo assistito a una tendenza generale che va dalle integrazioni batch alle metodologie di integrazione in tempo reale.
...
<table> 
 ...
 <tr>
  <td> <p>Svantaggi: </p></td>
  <td> <p>Aggiunge una chiamata al Audience Manager  dalla pagina.</p></td>
 </tr> 
</table>
...
* **Considerazioni tecniche: la consegna dei** dati dipende dalle capacità tecniche del partner dati.  Audience Manager può inviare/ricevere dati in tempo reale dal browser o mediante aggiornamenti batch attraverso processi di comunicazione offline server-to-server.
...
1. La terza parte restituisce al Audience Manager le informazioni sul segmento relative a tale utente.
...
Per informazioni che descrivono gli intervalli di tempo in cui  Audience Manager elabora i trasferimenti di file in entrata e in uscita [!DNL Server-to-Server], vedere [!UICONTROL S2S]Linee guida sui tempi di trasferimento dei file e dei rapporti[.](../reference/reporting-file-transfer-timeframe.md)
